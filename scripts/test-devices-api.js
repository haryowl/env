...
const testDevicesAPI = async () => {
  try {
    console.log('Testing devices API logic...');

    // Test for different users
    const testUsers = [
      { user_id: 1, username: 'admin', role: 'super_admin' },
      { user_id: 10, username: 'admin0', role: 'viewer' },
      { user_id: 5, username: 'admins', role: 'viewer' }
    ];

    for (const testUser of testUsers) {
      console.log(`\n=== Testing devices for ${testUser.username} (${testUser.role}) ===`);
      
      // Simulate the devices API query logic
      const conditions = [];
      const params = [];
      let paramCount = 1;
      ...
      try {
        ...
        devices.rows.forEach(device => {
          ...
          console.log(`    User permissions: ${JSON.stringify(device.user_permissions)}`);
          console.log(`    Role permissions: ${JSON.stringify(device.role_permissions)}`);
        });
      } catch (queryError) {
        console.error(`✗ Query failed for ${testUser.username}:`, queryError.message);
        console.error('Query:', devicesQuery);
